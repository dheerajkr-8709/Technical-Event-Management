  ...
  return (
    <div className="auth-container">
      <div className="auth-card">
        ...
        <div className="auth-title">Admin Sign Up</div>
        <div className="auth-subtitle">Create an admin account</div>
        {error && <div className="message error">{error}</div>}
        {success && <div className="message success">{success}</div>}
        <form onSubmit={handleSubmit}>
          <div className="form-row">
            <span className="form-label">Name</span>
            <input type="text" className="form-input" placeholder="Admin" value={name} onChange={e => setName(e.target.value)} />
          </div>
          <div className="form-row">
        ...
        <p style={{ textAlign: 'center', marginTop: '1.5rem', color: '#adb5bd', fontSize: '0.9rem' }}>
          Already have an account? <Link to="/admin/login" style={{ color: '#4895ef' }}>Login</Link>
        </p>
      </div>
    </div>
  );
