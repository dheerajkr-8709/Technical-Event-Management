...
export default function VendorLogin() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await axios.post('http://localhost:5000/api/auth/vendor/login', { email, password });
  ...
  return (
    <div className="auth-container">
      <div className="auth-card">
        ...
        <div className="auth-title">Vendor Login</div>
        <div className="auth-subtitle">Access your vendor panel</div>
        ...
        <p style={{ textAlign: 'center', marginTop: '1.5rem', color: '#adb5bd', fontSize: '0.9rem' }}>
          Don't have an account? <Link to="/vendor/signup" style={{ color: '#4895ef' }}>Sign Up</Link>
        </p>
      </div>
    </div>
  );
